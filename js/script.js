...
addToCartButtons.forEach((button) => {
  button.addEventListener('click', function (e) {
    ...
    const priceText = productCard.querySelector('.product-price').childNodes[0].textContent;
...
modalButtons.forEach((button) => {
  button.addEventListener('click', function (e) {
    ...
    const priceText = modalContent.querySelector('.product-price').childNodes[0].textContent;
    const cleanedPrice = priceText.replace('IDR', '').replace('K', '').trim();
    const price = parseInt(cleanedPrice) * 1000;
    const itemExist = cart.find((item) => item.name === name);
    if (itemExist) {
...
//Shopping-Cart Menu
const menuButtons = document.querySelectorAll('.menu-card .add-to-cart');
